...
func TestManagementUsageRequiresManagementAuthAndPopsArray(t *testing.T) {
	...
	if errUnmarshal := json.Unmarshal(legacyRR.Body.Bytes(), &usagePayload); errUnmarshal != nil {
		t.Fatalf("unmarshal usage response: %v body=%s", errUnmarshal, legacyRR.Body.String())
	}
	if _, ok := usagePayload["usage"]; !ok {
		t.Fatalf("usage response missing usage field: %s", legacyRR.Body.String())
	}
...
func TestModelsWithClientVersionReturnsCodexCatalog(t *testing.T) {
	...
	gpt55Priority := codexClientTestPriority(gpt55["priority"])
	if customPriority <= gpt55Priority {
		t.Fatalf("custom priority = %d, want greater than template priority %d", customPriority, gpt55Priority)
	}
